...
fn greedy_find_next_position(
    starts: &[usize],
    goals: &[usize],
    map: &[Tile],
    line_size: i32,
) -> Option<(usize)> {
    let mut dists_from_a_goal = HashMap::<usize, (usize, usize)>::new();
    let mut nodes_to_be_treated = goals
        .to_vec()
        .iter()
        .map(|x| (*x, 0usize))
    ...
    //  position, weight, parent
    let mut candidates = Vec::new();
    let neighbors = &[line_size, 1, -1, -line_size];

    for goal in goals {
        dists_from_a_goal.insert(*goal, (0, *goal));
    }

    'main: while !nodes_to_be_treated.is_empty() {
        nodes_to_be_treated
            .sort_by(|(_, node_value_a), (_, node_value_b)| node_value_b.cmp(node_value_a));
        let (node, _) = nodes_to_be_treated.pop().unwrap();
        let (node_value, node_parent) = dists_from_a_goal[&node];
        ...
        if starts.contains(&node) {
            ...
            candidates.push((node, node_parent));
            continue 'main;
        }
        ...
        for offset in neighbors {
            ...
            if let Some((p_value, p_parent)) = dists_from_a_goal.get_mut(&neighbor) {
                if *p_value < (node_value + 1)
                    || *p_value == (node_value + 1) && *p_parent <= node_parent
                {
                    continue;
                } else if *p_value == (node_value + 1) && *p_parent < node_parent
                    || *p_value > (node_value + 1)
                {
                    *p_parent = node_parent;
                    continue;
                } else if *p_value > (node_value + 1) {
                    *p_parent = node_parent;
                    *p_value = node_value + 1;
                    continue;
                }
            ...
            dists_from_a_goal.insert(neighbor, (node_value + 1, node_parent));
        }
    }

    if !candidates.is_empty() {
        candidates.dedup_by(|a, b| a.0 == b.0 && a.1 == b.1);
        candidates.sort_by(|(_, node_from_a), (_, node_from_b)| node_from_b.cmp(&node_from_a));

        let candidate_from = candidates.last().unwrap().1;

        let mut test = candidates
            .iter()
            .cloned()
            .filter(|(_, node_from)| *node_from == candidate_from)
            .collect::<Vec<(usize, usize)>>();
        test.sort_by(|(node_to_a, _), (node_to_b, _)| node_to_b.cmp(&node_to_a));

        return Some(test.pop().unwrap().0);
    }
...
fn move_attack(
    ...
    if enemies
        .iter()
        .all(|enemy| !in_range(attacker.position, enemy.position, line_size))
    {
        let starts = neighbors
            .iter()
            .map(|n| (n + attacker.position as i32) as usize)
            .filter(|&posi| map[posi] == Tile::Empty)
            .collect::<Vec<usize>>();
        ...
        for enemy in enemies.iter() {
            ends.append(
                &mut neighbors
                    .iter()
                    .map(|n| (n + enemy.position as i32) as usize)
                    .filter(|&posi| map[posi] == Tile::Empty)
                    .collect::<Vec<usize>>(),
            );
        }
    ...
    {
        let attack_on_titan = enemies
            .iter_mut()
            .enumerate()
            .filter(|(_, enemy)| in_range(attacker.position, enemy.position, line_size))
            .min_by(|(_, enemy_a), (_, enemy_b)| {
...
pub fn first_star() -> Result<(), Box<Error + 'static>> {
        ...
        for index in 0..map.len() {
            ...
            match tile {
                ...
                Elf => {
                    ...
                    let mut elf = elves.iter_mut().find(|elf| elf.position == index).unwrap();
    ...
    let total_hp = surviving_team
        .iter()
        .fold(0, |acc, survivor| acc + survivor.hp);
...
pub fn second_star() -> Result<(), Box<Error + 'static>> {
    ...
    let mut round;
    let mut base_elf_power = 18;
    let surviving_team;

    'main: loop {
        base_elf_power += 1;
        let (mut elves_sub, mut goblins_sub, mut map_sub) =
            (elves.clone(), goblins.clone(), map.clone());
        ...
        loop {
            ...
            for index in 0..map_sub.len() {
                ...
                match tile {
                    ...
                    Elf => {
                        //Stop condition
                        if goblins_sub.is_empty() && elves_sub.len() == elves.len() {
                            // visualize(&map_sub, line_size as usize);
