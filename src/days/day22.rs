...
fn visualize_2(map: &[bool], line_size: usize) {
    let mut txt = "".to_string();
    for (index, i) in map.iter().enumerate() {
        let temp = match i {
            true => "O",
...
fn dijkstra(map: &[u64], line_size: usize, target: (usize, usize)) -> u64 {
    ...
    let maps = [
        map.iter().map(|&x| x != 0).collect::<Vec<bool>>(),
        map.iter().map(|&x| x != 1).collect::<Vec<bool>>(),
        map.iter().map(|&x| x != 2).collect::<Vec<bool>>(),
    ];
    ...
    while !visitable_nodes.is_empty() {
        ...
        // Right
        if node.0 % line_size < line_size - 2 && maps[node.1][node.0 + 1] {
            let next_node = (node.0 + 1, node.1);
            let next_dist = dist + 1;
            if insert_distance_score(
                &visited_nodes,
    ...
    distance_scores[&(target_index, 1)]
}

fn insert_distance_score(
    visited_nodes: &HashSet<(usize, usize)>,
    visitable_nodes: &mut Vec<(usize, usize)>,
    distance_scores: &mut HashMap<(usize, usize), u64>,
...
pub fn second_star() -> Result<(), Box<Error + 'static>> {
    let values = prepare_input();
    // Shortest path may go beyond target, so we need to expand the map, a lot.
    let map = create_map(
        values[0],
        (values[1], values[2]),
    ...
    let distance = dijkstra(&map, values[1] + 100, (values[1], values[2]));
